...
public interface AccountRepository extends JpaRepository<Account, UUID> {
    ...
    @Query("SELECT a FROM Account a LEFT JOIN FETCH UserAccount ua on a = ua.userAccountKey.accountId WHERE ua.userAccountKey.userId = :user AND a.isBlocked = :isBlocked")
    Page<Account> findAllByUserIdWithPaginationAndStatus(@Param("user") User user, boolean isBlocked, Pageable pageable);
}
